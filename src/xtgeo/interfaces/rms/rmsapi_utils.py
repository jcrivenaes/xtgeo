...
if TYPE_CHECKING:
    if rmsapi is not None:
        try:
            from rmsapi import GeometryType, Project, VerticalDomain
        except ImportError:
            GeometryType = Any  # type: ignore[misc]
            Project = Any  # type: ignore[misc]
            VerticalDomain = Any  # type: ignore[misc]
    else:
        GeometryType = Any  # type: ignore[misc]
        Project = Any  # type: ignore[misc]
        VerticalDomain = Any  # type: ignore[misc]
...
__all__ = [
    "RmsApiUtils",
    "RoxUtils",  # For backward compatibility
    "_DomainType",
    "_DomainTypeClipBoardGeneral2D",
...
class RmsApiUtils:
    ...
    def __init__(self, project: RmsProjectType, readonly: bool = False) -> None:
        ...
        # After this check, rmsapi is guaranteed to be not None
        assert rmsapi is not None  # help mypy understand this
        ...
        if project is not None and isinstance(project, str):
            projectname = project
            assert rmsapi is not None  # guaranteed by __init__
