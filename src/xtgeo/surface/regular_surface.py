    ...
    def to_rms(
        ...
        Args:
            ...
            category: Required for horizons/zones: e.g. 'DS_extracted'. For
                clipboard/general2d_data is represents the folder(s), where "" or None
            ...
            domain: Default is 'depth', but 'time', 'unknown' is also possible.
                Note that domain only applies to stype = 'clipboard' and
                'general2d_data', since the domain for horizons/zones is defined
                more rigid when the horizon/zone category is created in RMS.
        ...
        """
        # Prepare a geometry/values snapshot safe for RMS (swap if yflip == -1)
        use_srf = self
        if self.yflip == -1:
            use_srf = self.copy()
