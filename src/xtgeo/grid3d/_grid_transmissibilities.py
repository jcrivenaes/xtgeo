...
def get_nnc_nested_hybrid(
    ...
    ``fmu.tools.nestedhybridgrid.create_nested_hybrid_grid``) provides the
    topological mapping of which mother cells connect to which refined cells
    and through which face direction.  This function computes the TPFA
    transmissibility for each pair using the Sutherland–Hodgman face-overlap
    algorithm for area and distance calculations.

    Args:
        grid: The merged nested-hybrid grid.
        permx: Permeability in I-direction (GridProperty or scalar).
        permy: Permeability in J-direction (GridProperty or scalar).
        permz: Permeability in K-direction (GridProperty or scalar).
        ntg: Net-to-gross ratio (GridProperty or scalar).
        nnc_table: DataFrame with columns ``I1, J1, K1`` (mother cell,
            1-based), ``I2, J2, K2`` (refined cell, 1-based), and
    ...
    F = _internal.grid3d.CellFaceLabel
    # Map DIRECTION string → (cell1 face label, cell2 face label, dir_tuple)
    _DIR_MAP: dict[
        str,
        tuple[
            "_internal.grid3d.CellFaceLabel",
        ...
        "I+": (F.East, F.West, (1, 0, 0)),
        "I-": (F.West, F.East, (-1, 0, 0)),
        "J+": (F.North, F.South, (0, 1, 0)),
        "J-": (F.South, F.North, (0, -1, 0)),
        "K+": (F.Bottom, F.Top, (0, 0, 1)),
        "K-": (F.Top, F.Bottom, (0, 0, -1)),
    }

    rows: list[dict] = []
    refined_boundary_ijk: set[tuple[int, int, int]] = set()

    for _, row in nnc_table.iterrows():
        direction = str(row["DIRECTION"]).strip()
        if direction not in _DIR_MAP:
            continue
        face1_label, face2_label, dir_tuple = _DIR_MAP[direction]

        # 0-based indices
        i1, j1, k1 = int(row["I1"]) - 1, int(row["J1"]) - 1, int(row["K1"]) - 1
        i2, j2, k2 = int(row["I2"]) - 1, int(row["J2"]) - 1, int(row["K2"]) - 1

        # Bounds check
        if not (0 <= i1 < nc and 0 <= j1 < nr and 0 <= k1 < nl):
            continue
        if not (0 <= i2 < nc and 0 <= j2 < nr and 0 <= k2 < nl):
            continue

        cc1 = gcpp.get_cell_corners_from_ijk(i1, j1, k1)
        cc2 = gcpp.get_cell_corners_from_ijk(i2, j2, k2)

        fr = _internal.grid3d.face_overlap_result(cc1, face1_label, cc2, face2_label)
        if fr.area <= 0.0:
            continue

        ke1 = _nh_k_eff(dir_tuple, (i1, j1, k1), px, py, pz, nt)
        opp = (-dir_tuple[0], -dir_tuple[1], -dir_tuple[2])
        ke2 = _nh_k_eff(opp, (i2, j2, k2), px, py, pz, nt)
        T = _nh_tpfa(ke1, ke2, fr.area, fr.d1, fr.d2, factor)
        ...
        refined_boundary_ijk.add((i2, j2, k2))
