...
class _GridCache:
    ...
    def _initialize(self, grid: Grid) -> None:
        ...
        self.onegrid_cpp = _internal.grid3d.Grid(self.onegrid)
...
class Grid(_Grid3D):
    ...
    def _get_cache(self) -> _GridCache:
        """Get the grid cache object.

        The cache should not be updated if it already exists for a given grid, using
        the hash to identigy if the grid instance has changed.
        """
        if not hasattr(self, "_cache"):
            self._cache = None

        if self._cache is not None:
            # check if the cache is still valid by comparing grid hash
            if self._cache.hash != hash(self):
