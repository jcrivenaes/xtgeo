...
/*
 * Geometric centre of a cell: simple average of its 8 corners.
 * Used for TPFA half-distance d_i = |n · (face_centroid − cell_centre)|.
 */
static Eigen::Vector3d
cell_centre(const CellCorners &c)
{
    return (c.upper_sw.data() + c.upper_se.data() + c.upper_nw.data() +
            c.upper_ne.data() + c.lower_sw.data() + c.lower_se.data() +
            c.lower_nw.data() + c.lower_ne.data()) *
           0.125;
}
                    ...
                    int coord_axis)
{
    ...
    Eigen::Vector3d cc1 = cell_centre(cell1);
    Eigen::Vector3d cc2 = cell_centre(cell2);
