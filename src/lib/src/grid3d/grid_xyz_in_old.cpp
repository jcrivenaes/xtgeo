                  ...
                  const int nrow)
{
    ...
    // if all of these are Nan, just return -1
...

{
    ...
    for (size_t idx = 0; idx < num_points; ++idx) {
        ...
        // sometimes, an ij-range is OK; but the get_proposed_ij fails to detect
        // the K column. The backup is then to to loop imin..imax, jmin..jmax instead.
        // But when a proposed is found we can narrow down the search to one column
        if (i_est >= 0) {
            imin = i_est;
            imax = i_est;
            jmin = j_est;
            ...
            for (int j = jmin; j <= jmax; ++j) {
                ...
                for (int offset = 0; offset < grid.nlay; ++offset) {
                    ...
                    if (k_up >= 0 && k_up < grid.nlay) {
                        ...
                        if ((!active_only ||
                             (active_only && actnumsv_(i, j, k_up) > 0)) &&
                            is_point_in_cell(point, cell_corners)) {
                            i_indices_(idx) = i;
                            j_indices_(idx) = j;
                            k_indices_(idx) = k_up;
                    ...
                    if (k_down >= 0 && k_down < grid.nlay) {
                        auto cell_corners =
                          get_cell_corners_from_ijk(grid, i, j, k_down);
                        if ((!active_only ||
                ...
                if (found) {
                    break;  // Break out of the J loop if a cell is found
                }
            }
            if (found) {
                break;  // Break out of the I loop if a cell is found
            }
        }
