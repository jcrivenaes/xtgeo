...
/*
 * Compute cube statistics per column, version 2, returning 2D arrays of min, max, mean,
 ...
 */
std::unordered_map<std::string, py::array_t<double>>
cube_stats_along_z_v2(const Cube &cube,
                      ...
                      int max_index)
{
    ...
    logger.debug("Computing cube statistics along Z axis (version 2)");
    size_t ncol = cube.ncol;
    size_t nrow = cube.nrow;
    size_t nlay = cube.nlay;
    ...
    for (size_t i = 0; i < ncol; i++) {
        for (size_t j = 0; j < nrow; j++) {
            ...
            if (lower_z - upper_z <= min_thickness) {
                minv_(i, j) = numerics::QUIET_NAN;
                maxv_(i, j) = numerics::QUIET_NAN;
                meanv_(i, j) = numerics::QUIET_NAN;
                varv_(i, j) = numerics::QUIET_NAN;
