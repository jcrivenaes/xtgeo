 ...
 * several methods exist. Several methods are implemented in this file.
 */
                                    ...
                                    const double tolerance_scaler)
{
    ...
    const double epsilon = 1e-6 * diagonal * tolerance_scaler;  // Relative tolerance
...
ray_intersects_triangle(const xyz::Point &origin,
                        ...
                        const double tolerance_scaler)
{
    const double epsilon = 1e-5 * tolerance_scaler;
...
// =====================================================================================
/**
 * @brief Special method for difficult cases, where the hexahedron is non-convex.
 * This method dynamically decomposes the hexahedron into tetrahedrons and checks if
 * the point is inside any of them. It uses the centroid of the hexahedron as a
 * reference point for the tetrahedrons. This method is more robust for non-convex
 * hexahedra, but less efficient than the signed volume method.
 */
static bool
                                  ...
                                  const double tolerance_scaler)
{
    ...
    for (const auto &tetra : dynamic_tetrahedrons) {
        if (is_point_in_tetrahedron(point, vertices[tetra[0]], vertices[tetra[1]],
                                    vertices[tetra[2]], centroid, tolerance_scaler)) {
            return true;
        }
    }
 ...
 * This method decomposes the hexahedron into X tetrahedrons and checks if the point is
 * inside any of them. It uses the signed volume method to determine if the point is
 * inside the hexahedron. This method is efficient and works well for convex
 * hexahedra, but is sometimes less reliable for points near edges or faces.
 */
static bool
                                           ...
                                           const double tolerance_scaler)
{
    ...
    const std::array<std::array<int, 4>, 5> tetrahedrons = { {
      { 0, 1, 3, 7 },  // Tetrahedron 1
      { 1, 5, 7, 0 },  // Tetrahedron 2
      { 1, 6, 7, 5 },  // Tetrahedron 3
      { 1, 2, 7, 6 },  // Tetrahedron 4
      { 1, 3, 7, 2 }   // Tetrahedron 5
    } };

    // Compute relative tolerance based on the hexahedron's size
    double diagonal =
      std::sqrt(std::pow(max_pt.x - min_pt.x, 2) + std::pow(max_pt.y - min_pt.y, 2) +
                std::pow(max_pt.z - min_pt.z, 2));
    double epsilon = 1e-10 * diagonal * tolerance_scaler;

    // For each tetrahedron, check if the point is inside using barycentric coordinates
    for (const auto &tetra : tetrahedrons) {
        ...
        const xyz::Point &v3 = vertices[tetra[3]];

        // Calculate barycentric coordinates
        double volume = signed_volume(v0, v1, v2, v3);

        // Skip degenerate tetrahedrons
        ...
        // Calculate the four signed volumes with the point
        double vol0 = signed_volume(point, v1, v2, v3) * inv_volume;
        double vol1 = signed_volume(v0, point, v2, v3) * inv_volume;
        double vol2 = signed_volume(v0, v1, point, v3) * inv_volume;
        double vol3 = signed_volume(v0, v1, v2, point) * inv_volume;

        printf("volumes: %f %f %f %f\n", vol0, vol1, vol2, vol3);

        // The point is inside if all volumes have the same sign (sum to 1)
        // and all are between 0 and 1 (with small epsilon tolerance)
        if (vol0 >= -epsilon && vol1 >= -epsilon && vol2 >= -epsilon &&
            vol3 >= -epsilon && std::abs(vol0 + vol1 + vol2 + vol3 - 1.0) < epsilon) {
            return true;
        }
    }
...
/**
 * Determines if a point is inside a hexahedron (8-vertex cell) using thetrahedrons.
...
// =====================================================================================
// Main function for point-in-cell test. The method can be selected.
// =====================================================================================
 ...
 * @param tolerance_scaler A scaling factor for the tolerance
 * @return true if the point is inside the hexahedron, false otherwise
                       ...
                       const double tolerance_scaler)
{
    ...
    switch (method) {
        case PointInHexahedronMethod::RayCasting:
            return is_point_in_hexahedron_using_raycasting(point, vertices,
                                                           tolerance_scaler);
        case PointInHexahedronMethod::Tetrahedrons:
            return is_point_in_hexahedron_using_tetrahedrons(point, vertices,
                                                             tolerance_scaler);
        case PointInHexahedronMethod::SignedVolumes:
            return is_point_in_hexahedron_using_signed_volume(point, vertices, min_pt,
                                                              max_pt, tolerance_scaler);

        case PointInHexahedronMethod::NonConvex:
            return is_point_in_non_convex_hexahedron(point, vertices, tolerance_scaler);
        case PointInHexahedronMethod::UsingPlanes:
            return is_point_in_hexahedron_using_planes(point, vertices, min_pt, max_pt,
                                                       tolerance_scaler);
        case PointInHexahedronMethod::ScoreBased:
            // evaluate using 4 methods and return the majority, except for cells that
            // are clearly concave in projected plane, or very thin
            logger.debug("Using score-based method for point-in-hexahedron test");
            if (is_hexahedron_thin(hexahedron_corners, 1e-6)) {
                // Check if the cell is thin, using planes
                return is_point_in_hexahedron_using_planes(point, vertices, min_pt,
                                                           max_pt, tolerance_scaler);
            }
            ...
            count += is_point_in_hexahedron_using_raycasting(point, vertices,
                                                             tolerance_scaler);
            count += is_point_in_hexahedron_using_tetrahedrons(point, vertices,
                                                               tolerance_scaler);
            count +=
              is_point_in_non_convex_hexahedron(point, vertices, tolerance_scaler);
            count += is_point_in_hexahedron_using_planes(point, vertices, min_pt,
                                                         max_pt, tolerance_scaler);

            logger.debug("Count is {}", count);
            return count >= 2;  // Majority rule
        default:
            throw std::invalid_argument("Invalid method for point-in-hexahedron test");
    }
