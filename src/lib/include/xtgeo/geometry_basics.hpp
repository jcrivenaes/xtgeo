...
/** Mathematic helper functions for geometry */
...
inline double
dN_dxi(int i, double eta, double zeta)  // Changed function name to match usage
{
...
inline double
dN_deta(int i, double xi, double zeta)  // Changed function name to match usage
{
...
inline double
dN_dzeta(int i, double xi, double eta)  // Changed function name to match usage
{
...
calculate_jacobian(const Point local_pt, const Point hex_vertices[8])
{
    ...
    for (int i = 0; i < 8; ++i) {
        double dNi_dxi = dN_dxi(i, local_pt.y, local_pt.z);
        double dNi_deta = dN_deta(i, local_pt.x, local_pt.z);
        double dNi_dzeta = dN_dzeta(i, local_pt.x, local_pt.y);
